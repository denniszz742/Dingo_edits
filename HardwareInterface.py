...
class HardwareInterface():
    ...
    def create(self):
        for i in range(16):
            self.servo[i].actuation_range = 180
            self.servo[i].set_pulse_width_range(self.pwm_min, self.pwm_max)
    ...
    def set_actuator_postions(self, joint_angles):
        ...
        for leg_index in range(4):
            for axis_index in range(3):
                try:
                    self.servo[self.pins[axis_index,leg_index]].angle = self.servo_angles[axis_index,leg_index]
                except:
